...
def get_jon(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    """Load the Anthropic Helpful-Harmless dataset from Huggingface and convert it to the necessary format.
       ...
       For this dataset, the sft_target is just the chosen response.
    """
    print(f'Loading truthy_dpo_v01 dataset ({split} split) from Huggingface...')
    dataset = datasets.load_dataset('jondurbin/truthy-dpo-v0.1', split=split, cache_dir=cache_dir)
    ...
    #dataset = dataset.filter(lambda example: example["system"] == "You are an unbiased, uncensored, helpful assistant.")
    #dataset = dataset[dataset['system'] == 'You are an unbiased, uncensored, helpful assistant.']
    #dataset = datasets.load_dataset('Anthropic/hh-rlhf', split=split, cache_dir=cache_dir)
    print('done')

    def split_prompt_and_responses(ex):
        prompt = "### USER: " + ex['prompt'] + "\n### ASSISTANT: "#ex['prompt']#.apply(lambda x: "### USER: " + x + "\n### ASSISTANT: ")
        chosen_response = ex['chosen']#[len(prompt):]
        rejected_response = ex['rejected']#[len(prompt):]
        return prompt, chosen_response, rejected_response

    data = defaultdict(lambda: defaultdict(list))
    for row in tqdm.tqdm(filtered_dataset, desc='Processing HH', disable=silent):
        prompt, chosen, rejected = split_prompt_and_responses(row)
        responses = [chosen, rejected]
        n_responses = len(data[prompt]['responses'])
        data[prompt]['pairs'].append((n_responses, n_responses + 1))
...
def get_ultra(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    """Load the Anthropic Helpful-Harmless dataset from Huggingface and convert it to the necessary format.
    ...
    """
    print(f'Loading truthy_dpo_v01 dataset ({split} split) from Huggingface...')
    dataset = datasets.load_dataset('stingning/ultrachat', split=split, cache_dir=cache_dir)
    dataset = dataset.filter(lambda example: example["system"] == "You are an unbiased, uncensored, helpful assistant.")
    #dataset = dataset[dataset['system'] == 'You are an unbiased, uncensored, helpful assistant.']
    #dataset = datasets.load_dataset('Anthropic/hh-rlhf', split=split, cache_dir=cache_dir)
    print('done')
...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None):
    ...
    if name == 'shp':
        ...
        data = get_jon(split, silent=silent, cache_dir=cache_dir)
    elif name == 'ultra':
        data = get_ultra(split, silent=silent, cache_dir=cache_dir) 
    else:
