#!/usr/bin/env bash
# Launch one isolated Spot L4 to run the EQUIVALENCE GATE only.
# Builds the eval reference cache, re-evaluates the committed cell through the optimized
# path into a scratch dir, compares, then self-deletes. Touches no committed artifact.
...
VM_NAME="${E9_VM_NAME:-pdpo-e9-gen-l4}"
...
BUNDLE="$(mktemp "${TMPDIR:-/tmp}/pdpo-e9-gen-source.XXXXXX")"
BUNDLE_OBJECT="session-control/e9-gen-source-$(date -u +%Y%m%dT%H%M%SZ).tar.gz"
COPYFILE_DISABLE=1 tar -C "${ROOT_DIR}" -czf "${BUNDLE}" \
  requirements.txt \
  idle_watchdog.py \
  precompute_ref_logps.py \
  config.py \
  experiments/run_stage2_rr.py experiments/build_eval_ref_cache.py experiments/equivalence_gate.py experiments/run_e9_generation.py \
  ...
  scripts/gcp/stage2_gate_remote.sh scripts/gcp/e9_generation_remote.sh
...
ROOT="/root/PDPO"
OUT_DIR="\${ROOT}/experiments/e9_utility"
...
( while true; do
    sleep 60
    gcloud storage cp "\${OUT_DIR}/startup.log" "${BUCKET_URI}/experiments/e9_utility/startup.log" \
      >/dev/null 2>&1 || true
  done ) >/dev/null 2>&1 &
BOOT_SYNC_PID=\$!
...
gcloud storage cp "${BUCKET_URI}/${BUNDLE_OBJECT}" /tmp/pdpo-e9-gen-source.tar.gz
tar -xzf /tmp/pdpo-e9-gen-source.tar.gz -C "\${ROOT}"
cd "\${ROOT}"
bash scripts/gcp/e9_generation_remote.sh
EOF
...
echo "Launched ${VM_NAME}. E9 generation output: ${BUCKET_URI}/experiments/e9_utility/"
echo "  MANIFEST.json  prompts.jsonl  generations_<model>.jsonl  e9.log"
