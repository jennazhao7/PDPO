...
VM_NAME="${STAGE2_GATE_VM_NAME:-pdpo-stage2-gate-l4}"
...
BUNDLE="$(mktemp "${TMPDIR:-/tmp}/pdpo-stage2-gate-source.XXXXXX")"
BUNDLE_OBJECT="session-control/stage2-gate-source-$(date -u +%Y%m%dT%H%M%SZ).tar.gz"
COPYFILE_DISABLE=1 tar -C "${ROOT_DIR}" -czf "${BUNDLE}" \
  requirements.txt \
  idle_watchdog.py \
  precompute_ref_logps.py \
  config.py \
  experiments/run_stage2_rr.py experiments/build_eval_ref_cache.py experiments/equivalence_gate.py \
  ...
  scripts/gcp/stage2_gate_remote.sh
...
ROOT="/root/PDPO"
OUT_DIR="\${ROOT}/experiments/stage2_gate"
...
( while true; do
    sleep 60
    gcloud storage cp "\${OUT_DIR}/startup.log" "${BUCKET_URI}/experiments/stage2_gate/startup.log" \
      >/dev/null 2>&1 || true
  done ) >/dev/null 2>&1 &
BOOT_SYNC_PID=\$!
...
gcloud storage cp "${BUCKET_URI}/${BUNDLE_OBJECT}" /tmp/pdpo-stage2-gate-source.tar.gz
tar -xzf /tmp/pdpo-stage2-gate-source.tar.gz -C "\${ROOT}"
cd "\${ROOT}"
bash scripts/gcp/stage2_gate_remote.sh
EOF
...
echo "Launched ${VM_NAME}. Gate output: ${BUCKET_URI}/experiments/stage2_gate/"
echo "  GATE_RESULT.json  gate.log  timing.json  cache_build.log"
