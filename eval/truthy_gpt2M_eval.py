...
def generate_batch(model_id: str, prompts: List[str], tokenizer, device: str = "cuda", max_new_tokens: int = 256, batch_size: int = 8) -> List[str]:
    ...
    Args:
        ...
        tokenizer: Shared tokenizer to use for encoding/decoding (GPT-2-medium tokenizer)
...
# ---------- main ----------
def main():
    ap = argparse.ArgumentParser(description="Evaluate gpt2-medium DPO model against baseline")
    ap.add_argument("--model_a", default="Jennazhao7/gpt2-medium-dpo-m1",
                    help="Your DPO model (A)")
    ap.add_argument("--model_b", default="Setpember/Jon_GPT2M_DPO_props_epi_point5",
                    help="Baseline PROPS DPO model (B) - epsilon=0.5")
    ...
    ap.add_argument("--batch_size", type=int, default=8, help="Batch size for generation (reduce if OOM)")
    ...
    ap.add_argument("--out_csv", default="props_win_tie_results_gpt2M.csv")
    args = ap.parse_args()
    ...
    # 1.5) Load shared GPT-2-medium tokenizer for fair comparison
    print("\n🔧 Loading shared GPT-2-medium tokenizer...")
    shared_tokenizer = AutoTokenizer.from_pretrained("gpt2-medium", use_fast=True)
