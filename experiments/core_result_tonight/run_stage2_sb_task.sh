...
MODEL="${1:?Usage: run_stage2_sb_task.sh <model_id>}"
GPU_ID="${GPU_ID:-0}"
EPSILON="${EPSILON:-1.0}"
SEED="${SEED:-42}"
...
OUT_DIR="$OUT_ROOT/stage2_softbayes/${SAFE_MODEL}_stage2_sb_eps${EPSILON}_seed${SEED}"
...
if [[ -f "$MANIFEST" ]]; then
  echo "SKIP stage2_sb: already done -> $MANIFEST"
  exit 0
fi
