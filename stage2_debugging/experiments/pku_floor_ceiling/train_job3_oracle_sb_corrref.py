#!/usr/bin/env python3
"""
Job 3 — Oracle SB, Interpolated Reference (The Method).

Fresh LoRA on base model. Oracle SB weights from was_flipped.
Per-sample reference = w_i * M1 + (1-w_i) * base  (log-ratio space).

Three frozen models per batch: M2 (being trained), M1 (frozen), base (frozen).
M1 and base are loaded once and never updated.

Usage:
  CUDA_VISIBLE_DEVICES=2 python train_job3_oracle_sb_corrref.py \
...
def interpolated_ref_log_ratio(
    ...
    """
    w=1 (clean pair): fully trust M1 as reference → use M1 log-ratio
    w=0 (flipped):    distrust M1 → fall back to base log-ratio
    Interpolation is in log-ratio space (not logit space).
    """
...
# ── On-the-fly reference scoring ─────────────────────────────────────────────

@torch.no_grad()
def score_batch(model, ids_batch, p_lens, pad_id, device):
    """Score a batch of already-tokenised sequences."""
    return compute_response_logprobs(model, ids_batch.to(device), p_lens.to(device), pad_id)
...
def parse_args():
    ...
    p.add_argument("--ref_sanity_every", type=int, default=50,
                   help="Print interpolated ref sanity every N optimizer steps")
    return p.parse_args()
...
def main() -> int:
    ...
    print(f"=== Job 3: Oracle SB — Interpolated Reference ===")
    ...
    for epoch in range(args.epochs):
        for bidx, batch in enumerate(dl):
            ...
            # Compute reference logprobs from M1 and base (no grad needed)
            with torch.no_grad():
