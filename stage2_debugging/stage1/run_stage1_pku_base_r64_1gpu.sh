...
OUT_ROOT="${OUT_ROOT:-${ROOT_DIR}/stage2_debugging/stage1/results_base_r64_eps${EPS}_seed${SEED}}"
...
OUT_PKU="${OUT_ROOT}/pku_base_r64_eps${EPS}_s${SEED}"
...
echo "[INFO] Starting Stage 1 training on PKU with BASE model & HIGH RANK (r=64)..."
...
CUDA_VISIBLE_DEVICES=0 python -u lora/preprocessing/train_truthy_stage1_lora.py \
  ...
  --seed "${SEED}" \
  > "${LOG_DIR}/train_pku_base_r64_s${SEED}.log" 2>&1
...
CUDA_VISIBLE_DEVICES=0 python -u eval/eval_stage1_accuracy.py \
  ...
  --test_jsonl "${PKU_TEST}" \
  --out_json "${EVAL_DIR}/pku_base_r64_eps${EPS}_s${SEED}_eval.json" \
  --max_len "${EVAL_MAX_LEN}" \
  > "${LOG_DIR}/eval_pku_base_r64_s${SEED}.log" 2>&1

python - <<PY
import json, os
p = "${EVAL_DIR}/pku_base_r64_eps${EPS}_s${SEED}_eval.json"
if not os.path.exists(p):
    ...
    n   = d.get("n", "N/A")
    print(f"=== PKU Base M1 (Rank=64) Result ===")
    print(f"  Base model : {d.get('base_model', 'N/A')}")
    print(f"  Accuracy   : {acc}  (N={n})")
    print(f"  Above 50%? {'YES ✅' if isinstance(acc, float) and acc > 0.5 else 'NO ❌'}")
PY

echo "[DONE] Stage1 base r64 train+eval complete: ${OUT_ROOT}"
