...
EPS="${EPS:-1.0}"
SEED="${SEED:-42}"
# Original was 300 steps. 5000 pairs / 32 batch size = ~156 steps/epoch.
# To genuinely test a longer epoch (e.g. 5-6 epochs), we use 1000 steps.
...
OUT_ROOT="${OUT_ROOT:-${ROOT_DIR}/stage2_debugging/stage1/results_base_longepoch_eps${EPS}_seed${SEED}}"
...
OUT_PKU="${OUT_ROOT}/pku_base_longepoch_eps${EPS}_s${SEED}"
...
echo "[INFO] Starting Stage 1 training on PKU (D1 ONLY) with LONG EPOCHS"
...
CUDA_VISIBLE_DEVICES=0 python -u lora/preprocessing/train_truthy_stage1_lora.py \
  ...
  --seed "${SEED}" \
  > "${LOG_DIR}/train_pku_base_longepoch_s${SEED}.log" 2>&1
...
CUDA_VISIBLE_DEVICES=0 python -u eval/eval_stage1_accuracy.py \
  ...
  --test_jsonl "${PKU_TEST}" \
  --out_json "${EVAL_DIR}/pku_base_longepoch_eps${EPS}_s${SEED}_eval.json" \
  --max_len "${EVAL_MAX_LEN}" \
  > "${LOG_DIR}/eval_pku_base_longepoch_s${SEED}.log" 2>&1

python - <<PY
import json, os
p = "${EVAL_DIR}/pku_base_longepoch_eps${EPS}_s${SEED}_eval.json"
if not os.path.exists(p):
    print("[WARN] Eval file missing:", p)
else:
    d = json.load(open(p))
    acc = d.get("accuracy", "N/A")
    print(f"=== PKU Base (D1 ONLY - LONG EPOCH) M1 Result ===")
