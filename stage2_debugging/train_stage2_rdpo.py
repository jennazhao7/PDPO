#!/usr/bin/env python3
"""
Stage-2 rDPO (Robust DPO) training on D2.

rDPO (Chowdhury et al., ICML 2024) is an unbiased correction to the DPO loss
under label noise. Where cDPO applies label smoothing (a biased regularization),
rDPO rescales the per-sample loss to fully debias it:

    L_rDPO = (1/(1-2ε)) * [-log σ(β * logits)] - (ε/(1-2ε)) * [-log σ(-β * logits)]

In TRL's DPOTrainer this is activated by setting `loss_type="robust"` and
passing the noise parameter via the `label_smoothing` argument.

For our RR mechanism the correct noise value (assigned to label_smoothing)
is derived from the DP epsilon:
...
train_stage2_dpo.py and train_stage2_cdpo.py. This is a SINGLE-STAGE method —
it does NOT use M1 scores, does NOT relabel data, and does NOT run a two-stage pipeline.
"""
...
def gamma_from_rr_epsilon(rr_eps: float) -> float:
    ...
    This is the theoretically correct noise parameter value for an RR mechanism
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Stage-2 rDPO (Robust DPO) training"
    )
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Noise parameter γ ∈ (0, 0.5) for rDPO. "
...
def main() -> int:
    ...
    if args.out is None:
        ...
        suffix = args.output_suffix or "stage2_rdpo"
    ...
    print(f"[rDPO]  label_smoothing={label_smoothing:.4f}  ({smoothing_source})")
    print(f"[rDPO]  beta={args.beta}  loss_type=robust")
    ...
    # ── rDPO config ───────────────────────────────────────────────────────
    # The differences from standard DPO: loss_type="robust" and label_smoothing > 0.
    dpo_config = DPOConfig(
        ...
        # ── rDPO core ──
        beta=args.beta,
        loss_type="robust",               # ← KEY rDPO CHANGE
        label_smoothing=label_smoothing,  # ← KEY rDPO CHANGE
    ...
    print("[Train] Stage-2 rDPO (Robust DPO)")
    ...
    manifest = {
        ...
        "notes": "M2 = base + stage1 + stage2 (Stage-2 trained with rDPO / loss_type=robust)",
    }
    ...
    print(f"✅ Saved Stage-2 (rDPO) adapter to {args.out}")
