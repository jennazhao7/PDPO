...
mkdir -p outputs_new_models/evals_full
...
run_eval() {
    ...
    local out_json="outputs_new_models/evals_full/${name}_eval.json"
    local log="outputs_new_models/logs/eval_full_${name}.log"
    echo "Starting full eval for ${name} on GPU ${gpu}..."
