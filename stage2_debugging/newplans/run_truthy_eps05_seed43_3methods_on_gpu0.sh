...
OUT_ROOT="${OUT_ROOT:-${ROOT_DIR}/stage2_debugging/stage2_truthy/results_eps0.5_seed43_gpu0}"
...
run_one() {
  ...
  mkdir -p "${out_dir}"
  echo "[RUN] gpu=0 method=${method} out=${out_dir}" | tee -a "${train_log}"
...
echo "[DONE] Truthy eps0.5 seed43 3-method single-GPU job complete: ${OUT_ROOT}"
