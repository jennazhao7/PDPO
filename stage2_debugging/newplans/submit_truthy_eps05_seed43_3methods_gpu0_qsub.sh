...
SCRIPT="${ROOT_DIR}/stage2_debugging/newplans/run_truthy_eps05_seed43_3methods_on_gpu0.sh"
[ -x "${SCRIPT}" ] || chmod +x "${SCRIPT}"

for f in "${S1_ADAPTER}/adapter_model.safetensors" "${D2_JSONL}" "${TEST_JSONL}"; do
  [ -f "${f}" ] || { echo "[FATAL] Missing required file: ${f}"; exit 2; }
done

command -v qsub >/dev/null 2>&1 || { echo "[FATAL] qsub not found in PATH"; exit 2; }

QSUB_OUT="${ROOT_DIR}/stage2_debugging/newplans/qsub_s2_truthy_eps05_seed43_gpu0.out"
QSUB_ERR="${ROOT_DIR}/stage2_debugging/newplans/qsub_s2_truthy_eps05_seed43_gpu0.err"
JOB_NAME="s2_truthy_e05_s43_g0"

echo "[INFO] Submitting Truthy eps0.5 seed43 (SB/MAP/MLE) on GPU0..."
JOB_ID=$(qsub -terse \
  -q gpu@@jung_gpu \
  -l gpu_card=1 \
  -pe smp 6 \
