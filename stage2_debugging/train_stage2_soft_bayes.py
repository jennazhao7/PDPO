...
def parse_args() -> argparse.Namespace:
    ...
    # Soft Bayes params
    ap.add_argument("--tau", type=float, default=0.0, help="Temperature for sigmoid(Δ/τ). 0=auto-tune from data.")
    ap.add_argument("--beta", type=float, default=0.5, help="DPO-like sharpness in soft BCE")
    ap.add_argument("--w_clamp_lo", type=float, default=0.01)
    ap.add_argument("--w_clamp_hi", type=float, default=0.99)
    ap.add_argument("--delta_clamp", type=float, default=500.0, help="Clamp Δ/τ to [-v,v]")
    ...
    ap.add_argument("--save_steps", type=int, default=100, help="Save Stage-2 adapter every N optimizer steps.")
    ap.add_argument("--save_total_limit", type=int, default=2, help="Keep at most this many periodic checkpoints.")
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument(
        "--enforce_calibrated",
        action="store_true",
        help="Fail unless calibrated Soft-Bayes hyperparams are used (beta=0.5, lr=2.5e-5).",
    )
    ...
    ap.add_argument(
        "--target_modules",
        type=str,
        # LLaMA/OpenLLaMA: q_proj,k_proj,v_proj,o_proj,...
        # Pythia/GPT-NeoX: query_key_value,dense,dense_h_to_4h,dense_4h_to_h
    ...
    ap.add_argument(
        "--bf16",
        action="store_true",
        help="Enable bfloat16 training (required for Mistral to avoid fp16 NaN overflow).",
    )
...
def score_sequence(
    ...
    """Compute log P_model(response | prompt) as SUM of log-probs over response tokens.

    Uses sum (not mean) to match standard DPO theory and trl.DPOTrainer.
    """
...
class SoftBayesPairDataset(TorchDataset):
    """Dataset yielding (prompt_chosen_ids, prompt_rejected_ids, w_i, ref logprobs) for soft DPO."""
...
def collate_soft_bayes(batch: List[Dict], pad_id: int) -> Dict:
    ...
    ref_c = [b["ref_chosen_lp"] for b in batch]
    ref_r = [b["ref_rejected_lp"] for b in batch]
    ...
    return {
        ...
        "ref_chosen_lp": torch.tensor(ref_c, dtype=torch.float32),
        "ref_rejected_lp": torch.tensor(ref_r, dtype=torch.float32),
    }
...
def compute_response_logprobs(
    ...
    """
    Compute SUM of log P(response_token | prompt, preceding) for each example.
    Returns shape [B]. Uses sum (not mean) to match DPO theory.
    """
    outputs = model(input_ids=input_ids)
    ...
    # Sum log-prob over response tokens per example (matches DPO theory)
    masked_lp = token_log_probs * mask.float()
    return masked_lp.sum(dim=1)
...
def soft_bayes_loss(
    ...
    """
    Soft-label BCE on pairwise logistic WITH reference model subtraction:
    r_c = log π(chosen|x) - log π_ref(chosen|x)   (policy advantage, chosen)
    r_r = log π(rejected|x) - log π_ref(rejected|x) (policy advantage, rejected)
    m_i = r_c - r_r
    L_i = -[w_i * log σ(β m_i) + (1 - w_i) * log(1 - σ(β m_i))]
    """
    # Policy advantage over reference (standard DPO formulation)
    reward_chosen = pi_chosen - ref_chosen      # [B]
    reward_rejected = pi_rejected - ref_rejected  # [B]
    m = reward_chosen - reward_rejected            # [B]
...
def main() -> int:
    ...
    if args.enforce_calibrated:
        if abs(args.beta - 0.5) > 1e-12 or abs(args.lr - 2.5e-5) > 1e-12:
            raise ValueError(
                "Calibrated Soft-Bayes required but got "
                f"beta={args.beta}, lr={args.lr}. Expected beta=0.5, lr=2.5e-5."
            )
    ...
    # Score D2 with M1 using DPO implicit reward AND pre-compute reference logprobs
    # DPO reward = log π_M1(y|x) - log π_base(y|x) isolates LoRA signal, cancels length bias.
    # Ref logprobs = M1 (base+stage1) logprobs, since M2's reference is base+stage1.
    print("[M1] Scoring D2 pairs (DPO implicit reward) + computing reference logprobs...")
    deltas = []
    ref_chosen_lps = []
    ref_rejected_lps = []
    for i in range(N):
        # DPO implicit reward for delta computation (length-agnostic)
        ...
        d = dr_c - dr_r
        deltas.append(d)
        # Ref logprobs: M1 (base+stage1) raw logprobs for training loss
        m1.set_adapter("stage1")
        ...
        if i < 5 or (i + 1) % 100 == 0:
            print(f"  [{i+1}/{N}] dpo_c={dr_c:.4f} dpo_r={dr_r:.4f} Δ={d:.4f} ref_c={s_c:.4f} ref_r={s_r:.4f}")

    # Guardrail: if all deltas collapse to ~0, M1 signal is likely broken
    # (e.g., wrong adapter path/load), which would invalidate Soft-Bayes posteriors.
    max_abs_delta = max(abs(x) for x in deltas) if deltas else 0.0
    if max_abs_delta < 1e-8:
        raise RuntimeError(
            "All M1 deltas are ~0. This suggests M1 scoring did not produce signal; "
            "check stage1 adapter path/loading before continuing."
        )
    ...
    score_stats = {
        ...
        "tau": args.tau,
        "beta": args.beta,
        "m1_scoring_mode": "dpo_reward_plus_ref_logprobs",
        "enforce_calibrated": bool(args.enforce_calibrated),
    }
    ...
    # Build dataset (includes pre-computed reference logprobs)
    pair_ds = SoftBayesPairDataset(
    ...
    print("[M2] Merging Stage-1 adapter into base weights...")
    ...
    m2 = get_peft_model(m2, stage2_config, adapter_name="stage2")
    ...
    for epoch in range(args.epochs):
        ...
        # Flush leftover gradients when len(train_dl) is not divisible by GA.
